...
func (db *DB) CreateAccount(accountParams CreateAccountParams) (User, error) {
...
func (db *DB) UpdateAccount(accountParams UpdateAccountParams) (User, error) {
	res := db.conn.QueryRow(
...
func (db *DB) GetAccount(id int) (User, error) {
...
func (db *DB) GetAccountByEmail(email string) (User, error) {
	row := db.conn.QueryRow(
...
func (db *DB) GetRoomByExternalID(externalId string) (Room, error) {
	row := db.conn.QueryRow(
		"SELECT id, external_id, name, description, seq_id FROM rooms "+
...
func (db *DB) FetchRoomWithSubscribers(roomId int) (*Room, error) {
	query := `
		SELECT 
				r.id AS room_id,
				r.external_id,
...
func (db *DB) CreateRoom(params CreateRoomParams) (Room, error) {
...
func (db *DB) CreateSubscription(userId, roomId int) (Subscription, error) {
	res := db.conn.QueryRow(
		createSubQuery,
		userId,
		roomId,
...
func (db *DB) SubscriptionExists(account_id, room_id int) bool {
	res := db.conn.QueryRow(
...
func (db *DB) ListSubscriptions(account_id int) ([]Room, error) {
...
func (db *DB) DeleteSubscription(accountId, roomId int) error {
	_, err := db.conn.Exec(
...
func (db *DB) MessageCreate(msg UserMessage) error {
...
func (db *DB) RoomUpdateOnMessage(msg UserMessage) error {
	_, err := db.conn.Exec("UPDATE rooms SET seq_id = $1 WHERE id = $2", msg.SeqId, msg.RoomId)

	return err
}

func (db *DB) GetSubscribersForRoom(roomId int) ([]User, error) {
	rows, err := db.conn.Query(
...
func (db *DB) MessageGetAll(roomId, since, before, limit int) ([]UserMessage, error) {
